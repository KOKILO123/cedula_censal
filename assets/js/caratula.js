...
function getEstPenitenciarios(ccof_regional, estPenitenciario) {
    var oOptions = {
        type: 'POST',
        url: base_url + 'index.php/caratulaController/getEstPenitenciario',
        data: {'ccof_regional': ccof_regional}
    };
    var posting = $.ajax(oOptions);
    var $select = $('#' + estPenitenciario);
    $select.find('option').remove();
    posting.done(function (response, textStatus, jqXHR) {
        response = JSON.parse(response);
        $.each(response, function (key, value)
...
function getOfRegional(ccdd, OfRegional) {
    var oOptions = {
        type: 'POST',
        url: base_url + 'index.php/caratulaController/getOficRegional',
        data: {'ccdd': ccdd}
    };
    var posting = $.ajax(oOptions);
    var $select = $('#' + OfRegional);
    $select.find('option').remove();
    posting.done(function (response, textStatus, jqXHR) {
        response = JSON.parse(response);
        $.each(response, function (key, value)
...
$('#frm_CCOF_REGIONAL').change(function () {
    $('#frm_OF_REGIONAL').val($(this).val());
})

$('#frm_EST_PENIT_COD').change(function () {
    $('#frm_EST_PENIT').val($(this).val());
});

$('#frm_CCDD').change(function () {
    getProvincias(this.value, 'frm_CCPP', 'frm_CCDI');
    var ccdd = $('#frm_CCDD').val()
    getOfRegional(ccdd, 'frm_CCOF_REGIONAL');
});
...
//    var ubigeo = $('#frm_CCDD').val() + $('#frm_CCPP').val() + $('#frm_CCDI').val();
//    getCentosPoblados(ubigeo, 'frm_CCCP');
//});
...
$('#frm_CCOF_REGIONAL').change(function () {
    var ccof_regional = $('#frm_CCOF_REGIONAL').val();
    getEstPenitenciarios(ccof_regional, 'frm_EST_PENIT_COD');
});
...
//    var centro_poblado = $('#frm_CCDI option:selected').text();
...
//    $('#frm_CP').val((centro_poblado === 'SELECCIONE') ? '' : centro_poblado);
    form.submit();
}
