...
abstract class FormBase {
    ...
    public function setUsedFields($usedFields) {
        if (!in_array($usedFields, $this->usedFields)) {
    ...
    public function close_tag() {
        $vars = form_hidden('fvars', implode('|', $this->getUsedFields()));
        return $vars . '</form>';
    }
...
//        $data = array();
        ...
        $_fvars = $this->ci->input->post('fvars');
        $keys = explode('|', $_fvars);
...
//        exit;
    ...
    public function getEstadoMarco() {
        $ci = get_instance();
        $this->ci->load->model('marco');
//        return $model->getEstado();
        return $this->ci->marco->getEstado();
    }
...
//        }
//        if ($this->getEstadoMarco() == 2 || !$this->getEstadoMarco()) {
...
//        var_dump($field, $value);
        ...
        if (!in_array($type, array('checkbox',))) {
            $this->setValueOptional($field, trim($value));
        } else {
