...
//        get_instance()->load->helper('inflector');
    ...
    public function to_xml($data = null, $structure = null, $basenode = 'xml', $filename = NULL) {
        if ($data === null and !func_num_args()) {
            $data = $this->_data;
        }
        ...
        return ($filename) ? $structure->asXML($filename) : $structure->asXML();
    }
...
//                }
//            });
            array_walk_recursive($str, 'encodingToJson');
...
function encodingToJson (&$item, $key) {
    if (!mb_detect_encoding($item, 'utf-8', true)) {
        $item = utf8_encode($item);
    }
}