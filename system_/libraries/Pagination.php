...
//			$this->base_url = rtrim($this->base_url).'&amp;'.$this->query_string_segment.'=';
        ...
        function record_links()
        {
            $first = 1;
            $last = 1;
            $total = $this->total_rows;
            $num_pages = ceil($this->total_rows / $this->per_page);
//            var_dump($this->cur_page===1.0, $num_pages);
            if($num_pages>1) {
                if($num_pages===$this->cur_page){
                    $last = $this->total_rows;
                }else{
                    $last = $this->cur_page*$this->per_page;
                }
                if(1.0===$this->cur_page){
                    $first = 1;
                }else{
                    $first = (($this->cur_page-1)*$this->per_page)+1;
                }
            }
            return sprintf('Mostrando %s a %s de %s empresas', $first, $last, $total);
        }
}
